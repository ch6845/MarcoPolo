...
if idx%5==gpu_idx and os.path.exists('datasets/extract/{}.QQscore.runtime.tsv'.format(dataset_name))==False:
    print(idx,gpu_idx)
    dataset_name=dataset_name_all[idx]
    ...
    QQscore_result=QQ.save_QQscore(path=path,device='cuda:{}'.format(gpu_idx if gpu_idx<2 else gpu_idx+1),num_cluster_list=[1,2],num_thread=6)
    end_time=time.time()
    print('\n',end_time-start_time)
    pd.DataFrame([end_time-start_time]).to_csv('{}.QQscore.runtime.tsv'.format(path),index=None,header=None)
    allscore=summarizer.save_MarcoPolo(input_path=path,output_path=path)